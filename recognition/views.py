...
import time
from frs_face_recognition.settings import BASE_DIR
import os
...
def total_number_employees():
    qs = User.objects.all()
    # -1 to remove admin count from the total number of the employes present
    return (len(qs) - 1)
...
def mark_your_attendance(request):
    ...
    # path to the face recognition model is set here
    predictor = dlib.shape_predictor(
        'face_recognition_data/shape_predictor_68_face_landmarks.dat')

    # path to numpy classes
...
def mark_your_attendance_out(request):
    ...
    # Add path to the shape predictor
    # CHANGED TO RELATIVE PATH LATER
    predictor = dlib.shape_predictor(
        'face_recognition_data/shape_predictor_68_face_landmarks.dat')
    svc_save_path = "face_recognition_data/svc.sav"
